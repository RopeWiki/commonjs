...
function findAndAddDataToMap(map) {
    return new Promise(function(resolve) {
        ...
        var mainCoords = tryAddMarker("kmlmarker", pageName.replace(/_/g, " "), "https://maps.google.com/mapfiles/kml/paddle/grn-stars.png");
                    ...
                    function printLayerNames(layers) {
                        for (var key in layers) {
                            ...
                            if (layer.options && layer.options.name && layer.options.color) {
                                ...
                                var lengthStr = length > 0 ? " (" + (length / 1e3).toFixed(2) + " km)" : "";
                                legend.innerHTML += '<i style="background:' + layer.options.color + ';width:12px;height:12px;display:inline-block;"></i> ' + layer.options.name + lengthStr + "<br>";
                            }
        ...
        if (kmllistquery) {
            ...
            var urlQuery = SITE_BASE_URL + "/api.php?action=ask&format=json" + "&query=" + urlencode("[[Category:Canyons]][[Has coordinates::+]]" + queryRaw) + getLocationParameters(numberToLoad) + "|order=descending,ascending|sort=Has rank rating,Has name" + "|offset=0";
...
function addStaticTile(map) {
    ...
    const bounds = L.latLngBounds(map.unproject(topLeft, zoom), map.unproject(bottomRight, zoom));
    var staticUrl = "https://tf.coops.workers.dev/static/outdoors/" + center.lng.toFixed(5) + "," + center.lat.toFixed(5) + "," + zoom + "/" + width + "x" + height + ".jpg70";
