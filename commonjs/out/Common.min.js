...
function setConstants() {
    ...
    switch (local.baseurl) {
      ...
      default:
        ...
        GOOGLE_MAPS_APIKEY = "AIzaSyCjAsmHKkUuAZsvC0tIBWnhXa5QSp0GEmU";
        break;
...
function curposCreateButton() {
    ...
    currentPositionControl.innerHTML = '<div title = "My Location">' + '<i class="material-icons">&#xe55c;</i>' + "</div>";
...
var compassFocusZoom = 14;

function curposMarkerAnchorPt() {
...
function curposToggle(force) {
    if (!!force) curposShowing = force; else {
        if (!curposShowing) curposShowing = true; else if (!curposZoomed) curposZoomed = true; else curposShowing = curposZoomed = false;
    }
    if (curposZoomed && !!curposMarker) {
        ...
        if (!inView || zoomedOut) {
            ...
            if (zoomedOut) map.setZoom(compassFocusZoom);
            return;
        }
    ...
    if (curposShowing) {
        ...
        if (!!curposOriginalBounds) {
            map.fitBounds(curposOriginalBounds, 0);
...
function curposInitialize(position) {
    ...
    curposWatchId = navigator.geolocation.watchPosition(function(position) {
        curposCoords = new google.maps.LatLng(position.coords.latitude, position.coords.longitude);
        curposMarker.setPosition(curposCoords);
    }, curposHandleError, curposOptions);
}
...
function curposUpdateCompassMarker() {
    if (!curposCompassHeading || !curposCoords || map.getZoom() < compassMinZoom) {
        if (!!curposCompassMarker) {
            curposCompassMarker.setMap(null);
            curposCompassMarker = null;
        }
    ...
    var compassPointer = {
        path: "M 7 2 L 12 -4 L 17 2 L 12 0 L 7 2 z",
        strokeColor: "#00F",
        fillColor: "#007fff",
        fillOpacity: 1,
        anchor: curposMarkerAnchorPt(),
...
function curposGetMagneticDeclination(lat, lon) {
    ...
    var url = "https://www.ngdc.noaa.gov/geomag-web/calculators/calculateDeclination?lat1=" + lat + "&lon1=" + lon + "&key=zNEw7&resultFormat=json";
    $.getJSON(geturl(url), function(data) {
        if (data) {
...
function createDropdownControl(options) {
    ...
    selectedItem.addEventListener("click", function() {
        if (dropdown.style.display === "" || dropdown.style.display === "none") {
            ...
            setArrow(dropdown);
        } else hideDropdownList(dropdown);
    });
    document.addEventListener("click", function(event) {
        var targetElement = event.target || event.srcElement;
        if (targetElement.id !== basename + "-current" && targetElement.id !== basename && targetElement.parentElement && targetElement.parentElement.id !== basename + "-items-list" && targetElement.parentElement.parentElement && targetElement.parentElement.parentElement.id !== basename + "-items-list") {
            if (dropdown.style.display === "block") hideDropdownList(dropdown);
        }
    });
...
function setArrow(dropdown) {
    ...
    dropdown.arrowElement.src = !!dropdown.bottomUp === expanded ? PROTOCOL + "maps.gstatic.com/mapfiles/arrow-down.png" : PROTOCOL + "maps.gstatic.com/mapfiles/arrow-up.png";
}
...
function initFullscreenControl() {
    ...
    fullscreenControl.innerHTML = '<div title = "Toggle Fullscreen">' + '<div class="controls fullscreen-control icon top-left"></div>' + '<div class="controls fullscreen-control icon top-right"></div>' + '<div class="controls fullscreen-control icon bottom-left"></div>' + '<div class="controls fullscreen-control icon bottom-right"></div>' + "</div>";
...
function adjustControlsForFullscreen(fullscreenControl, isFullscreen) {
    if (isFullscreen) fullscreenControl.classList.add("is-fullscreen"); else fullscreenControl.classList.remove("is-fullscreen");
...
function toggleFullScreen(force) {
    ...
    if (toggleFS == null || force) {
        if (toggleFS == null) {
            ...
            if (window.location.href.toString().indexOf(FULLSCREEN_HASH) < 0) window.location.href += FULLSCREEN_HASH;
            list = document.body.childNodes;
            for (i = 0; i < list.length; ++i) if (list[i].tagName === "DIV") {
        ...
        list = document.body.childNodes;
        for (i = 0; i < list.length; ++i) if (list[i].normal_display !== undefined) list[i].style.display = list[i].normal_display;
...
var mapLayerDropdownHideTimer = true;
...
function setDefaultMapLayer() {
    ...
    if (map.mapTypes.get(defaultLayer) != null || defaultLayer === google.maps.MapTypeId.TERRAIN || defaultLayer === google.maps.MapTypeId.ROADMAP || defaultLayer === google.maps.MapTypeId.HYBRID || defaultLayer === google.maps.MapTypeId.SATELLITE) {
        map.setMapTypeId(defaultLayer);
    }
    mapTypeObserver.observe(document.getElementById("mapbox"), {
        attributes: true,
        childList: true,
        subtree: true
    });
...
function initLegendControl() {
    ...
    condenseButton.addEventListener("click", function() {
        toggleLegendWidth();
    });
...
function initResizeControl() {
    ...
    resizeControl.addEventListener("touchstart", function(e) {
        var ghostbar = createGhostbar();
...
function initSearchMapControl() {
    ...
    searchMapControl.addEventListener("click", function() {
...
function waitForElement(elementId) {
    return new Promise(function(resolve) {
        var observer = new MutationObserver(function(mutationsList) {
            for (var i = 0; i < mutationsList.length; i++) {
                var mutation = mutationsList[i];
                if (!!mutation.previousSibling && mutation.previousSibling.id === elementId || mutation.addedNodes.length > 0 && mutation.addedNodes[0].id === elementId) {
                    observer.disconnect();
                    resolve();
                    return;
                }
...
var searchMapBoundsChangedCallback = searchMapCallbackRegionSearch;

var clearLocationsUpdateTableCallback = updateTable;
...
function searchMapButtonClicked() {
    if (searchmapn < 0) {
        ...
        if (searchRectBounds !== map.getBounds()) setSearchMapRectangleBounds(false);
        setLoadingInfoText();
    } else {
...
function createAndDisplaySearchRectangle(bounds) {
    ...
    var searchButton = document.getElementById("searchinfo");
    if (searchButton !== null) setCancelText(); else {
...
function searchMapCallbackRegionSearch(checkCountOnly, bounds) {
    ...
    var query = "[[Category:Canyons]]" + "[[Has latitude::>" + sw.lat().toFixed(3) + "]]" + "[[Has longitude::>" + sw.lng().toFixed(3) + "]]" + "[[Has latitude::<" + ne.lat().toFixed(3) + "]]" + "[[Has longitude::<" + ne.lng().toFixed(3) + "]]";
...
function getBoundsForSearchRectangle() {
    ...
    var bounds = new google.maps.LatLngBounds(new google.maps.LatLng(searchRectLatBtm, searchRectLatLft), new google.maps.LatLng(searchRectLatTop, searchRectLatRgt));
    return bounds;
}
...
function toggleRoutes(kmlfile, kmlgroup) {
    if (downloadedRoutes.includes(kmlgroup)) {
        if (showRoutes == null) toggleRoutes(null, null);
        return;
    }
    ...
    if (!gxml || kmlfile) {
        function geoxmlinitr() {
            if (!gxml) gxml = new GeoXml("gxml", map, "", {
                nozoom: true,
                simplelegend: true,
        ...
        if (typeof GeoXml == "undefined") $.getScript(geturl(SITE_BASE_URL + "/index.php?title=MediaWiki:Geoxml.js&action=raw&ctype=text/javascript"), geoxmlinitr); else setTimeout(geoxmlinitr, 100);
        if (kmlfile) return;
    } else {
        if (showRoutes == null) {
            showRoutes = "on";
...
var trackSourceDropdownHideTimer = true;
...
function initTrackSourceControl(selection) {
    ...
    selection.forEach(function(link) {
        ...
        if (link === selection[0]) item.classList.add("selected");
    ...
    dropdown.style.maxHeight = mapbox.offsetHeight - 66 + "px";
}
...
function loadSource(link, domain) {
    ...
    var kmlfile = link;
    if (!isropewiki) kmlfile = LUCA_BASE_URL + "/rwr?gpx=off&filename=tmp&kmlnfx&kmlx=" + kmlfile;
...
function SetupMapLayers() {
    ...
    for (var i = 0; i < mapLayerIds.length; i++) {
        switch (mapLayerIds[i]) {
          ...
          case "topousa1":
            map.mapTypes.set("topousa1", new google.maps.ImageMapType({
                getTileUrl: function(p, z) {
                    return PROTOCOL + "services.arcgisonline.com/arcgis/rest/services/USA_Topo_Maps/MapServer/tile/" + z + "/" + slippyClip(p.y, z) + "/" + slippyClip(p.x, z);
                },
                maxZoom: 15,
                minZoom: 2,
                opacity: 1,
          ...
          case "topousa2":
            map.mapTypes.set("topousa2", new google.maps.ImageMapType({
                getTileUrl: function(p, z) {
                    return PROTOCOL + "basemap.nationalmap.gov/arcgis/rest/services/USGSTopo/MapServer/tile/" + z + "/" + slippyClip(p.y, z) + "/" + slippyClip(p.x, z);
                },
                maxZoom: 16,
                minZoom: 2,
                opacity: 1,
          ...
          case "topoworld":
            map.mapTypes.set("topoworld", new google.maps.ImageMapType({
                getTileUrl: function(p, z) {
                    return PROTOCOL + "tile.thunderforest.com/outdoors/" + z + "/" + slippyClip(p.x, z) + "/" + slippyClip(p.y, z) + ".png?apikey=bdbb04f2d5df40cbb86e9e6e1acff6f7";
                },
                maxZoom: 18,
                minZoom: 2,
                opacity: 1,
          ...
          case "estopo":
            map.mapTypes.set("estopo", new google.maps.ImageMapType({
                getTileUrl: function(p, z) {
                    return PROTOCOL + "www.ign.es/wmts/mapa-raster?SERVICE=WMTS&REQUEST=GetTile&VERSION=1.0.0&LAYER=MTN&STYLE=default&TILEMATRIXSET=GoogleMapsCompatible&TILEMATRIX=" + z + "&TILEROW=" + slippyClip(p.y, z) + "&TILECOL=" + slippyClip(p.x, z) + "&FORMAT=image%2Fjpeg";
                },
                maxZoom: 18,
                minZoom: 6,
                opacity: 1,
...
function GetMapLayerIds() {
    ...
    if (isSpain()) mapLayerIds.push("estopo");
...
function isUSA() {
    ...
    return pageName === "United_States" || parentRegionEnable;
}
...
function SetWeatherLayerType(showIaState) {
    if (showIaState) {
        weatherGetTileUrl = "return 'https://mesonet.agron.iastate.edu/cache/tile.py/1.0.0/nexrad-n0q-[time]/' + zoom + '/' + tile.x + '/' + tile.y + '.png';";
    } else {
        var colorScheme = 4;
        weatherGetTileUrl = "return 'https://tilecache.rainviewer.com/v2/radar/[time]/256/' + zoom + '/' + tile.x + '/' + tile.y + '/" + colorScheme + "/1_1.png';";
    }
}
...
function animateNextLayer() {
    var len = radarTimes.length;
    if (len === 0) return;
    if (!weatherLayersLoaded) loadWeatherLayers();
    var layer = radarStep;
    var startIndex = 0;
    ...
    if (layer + 1 <= len) {
        ...
        if (previousLayer < 0) previousLayer = len - 1;
        map.overlayMapTypes.getAt(previousLayer + startIndex).setOpacity(0);
        map.overlayMapTypes.getAt(layer + startIndex).setOpacity(.6);
    }
    radarStep++;
    if (radarStep > len + 6) {
        radarStep = 0;
...
function displayinfowindow(marker) {
    ...
    if (lastinfowindow) {
        if (lastinfowindow.map) {
            ...
            if (lastinfowindow === marker.infowindow) return;
        }
    }
...
function loadlist(list, fitbounds) {
    ...
    if (calcnearby) {
        for (i = 0; i < list.length; ++i) {
            ...
            var distlist = [];
            for (ic = 1; ic < sortlist.length && ic <= 5 && sortlist[ic].distance < 20; ++ic) distlist.push(sortlist[ic].id);
    ...
    for (i = 0; i < list.length; ++i) {
        ...
        if (!item.zindex) {
            --nlist;
            zindexm = 5e3 + nlist;
            if (item.stars) zindexm += item.stars * 1e3;
        } else {
            zindexm = item.zindex;
        }
        var iconm = "";
        ...
        if (item.description) {
            ...
            if (metric) {
                ...
                for (var p = 0; p < pk.length; ++p) {
                    ...
                    if (ps[0] >= "0" && ps[0] <= "9") {
                        var unit = ps.slice(-2);
                        if (unit === "mi") pk[p] = pre + uconv(ps, miStr); else if (unit === "ft") pk[p] = pre + uconv(ps, ftStr);
                    }
                }
                str = pk.join(" ");
            }
        ...
        if (kmladdbutton) contentString += '<input class="submitoff addbutton" title="Add to a custom list" type="submit" onclick="addToList(\'' + item.id.split("'").join("%27") + '\')" value="+">';
        ...
        var tooltipString = item.infodescription ? item.infodescription : '<b class="nostranslate">' + nonamespace(item.id) + "</b><br>" + descm.split("*").join("&#9733;");
...
function setSpecifiedLocationsQuery() {
    ...
    isSpecifiedListTable = function() {
        return true;
    };
    var elems = document.getElementsByClassName("slideshowchk");
    elems[0].parentElement.parentElement.style.display = "none";
}
...
function countLocationsWithinSearchArea() {
    ...
    for (var i = 0; i < markers.length; i++) {
        var marker = markers[i];
        if (bounds.contains(marker.position)) numLocations++;
    }
    return numLocations;
}
...
function initializemap() {
    if ($("#kmllistquery").length !== 0 || !!document.getElementById("waterflow-table") || isMapPage() || isNearbyPhotosPage()) loadInteractiveMap(); else loadStaticMap();
}
...
function loadInteractiveMap() {
    ...
    map.panTo(new google.maps.LatLng(30, -40));
    map.setZoom(2);
    SetupMapLayers();
    ...
    if (kmlrect != null) {
        ...
        if (coords != null && coords.length > 1) {
            kmlmap = "kmlrect";
            boundslist = new google.maps.LatLngBounds(new google.maps.LatLng(coords[0], coords[1]), new google.maps.LatLng(coords[2], coords[3]));
    ...
    if (kmllist != null) {
        ...
        for (var i = 0; i < list.length; ++i) {
            ...
            if (line.length > 3) {
                ...
                if (line.length > 6) obj.icon = KML_ICON_LIST[Number(line[6]) * 6 + obj.stars];
            }
            if (kmlsummary) if (obj.id[0] === "#") {
                var num = obj.id.slice(1).split(" ")[0];
                obj.icon = "https://sites.google.com/site/rwicons/bg" + obj.stars + "_" + num + ".png";
    ...
    if (kmlsummary != null) {
        function spiderfy(srepeat) {
            ...
            for (var r = 0; r < srepeat && overlap; ++r) {
                overlap = false;
                for (var i = 0; i < markers.length; i++) for (var j = i + 1; j < markers.length; j++) if (i != j) {
            ...
            for (var i = 0; i < markers.length; i++) {
                ...
                if (!isNaN(ll.lat) && !isNaN(ll.lng)) m.setPosition(ll); else console.log("error " + m.name + ":" + m.p + " -> " + ll.lat + "," + ll.lng);
            }
        }
    ...
    if (kmltitle != null) {
        ...
        titleControlsDiv.innerHTML = '<img src="https://chart.apis.google.com/chart?chst=d_text_outline&chld=000000|32|h|FFFFFF|b|' + urlencode(kmltitle.innerHTML) + '"/>';
    ...
    if (kmlfilep != null) {
        ...
        if (file != null && file.length > 0) {
            ...
            if (typeof GeoXml == "undefined") $.getScript(geturl(SITE_BASE_URL + "/index.php?title=MediaWiki:Geoxml.js&action=raw&ctype=text/javascript"), geoxmlinitp); else setTimeout(geoxmlinitp, 100);
        }
    }
    ...
    if (kmlfilew != null) {
        ...
        if (file != null && file.length > 0) {
            ...
            if (typeof GeoXml == "undefined") $.getScript(geturl(SITE_BASE_URL + "/index.php?title=MediaWiki:Geoxml.js&action=raw&ctype=text/javascript"), geoxmlinitw); else setTimeout(geoxmlinitw, 100);
        }
    }
...
mw.loader.load("https://unpkg.com/leaflet/dist/leaflet.js");

mw.loader.load("https://unpkg.com/leaflet/dist/leaflet.css", "text/css");

mw.loader.load("https://unpkg.com/leaflet-omnivore/leaflet-omnivore.min.js");
...
function addKMLMarkers(map) {
    ...
    if (kmlmarker != null) {
        ...
        if (coords != null && coords.length > 1) {
            map.setView(coords);
            addMarker(coords, map, pageName.replace(/_/g, " "), "https://maps.google.com/mapfiles/ms/icons/red-dot.png");
        }
    }
    var kmlmarkershuttle = document.getElementById("kmlmarkershuttle");
    if (kmlmarkershuttle != null) {
        ...
        if (coords != null && coords.length > 1) {
            addMarker(coords, map, "Shuttle", "https://maps.google.com/mapfiles/kml/paddle/S.png");
        }
    }
    var kmlmarkerparking = document.getElementById("kmlmarkerparking");
    if (kmlmarkerparking != null) {
        ...
        if (coords != null && coords.length > 1) {
            addMarker(coords, map, "Parking", "https://maps.google.com/mapfiles/kml/paddle/P.png");
        }
    }
...
function addMarker(coords, map, text, iconUrl) {
    if (coords != null && coords.length > 1) {
...
var tooltip = function() {
    ...
    return {
        show: function(v, w, highlight) {
            if (!v || v === "") return;
            if (highlight && lasthighlight) if (highlight.mouseoverhighlight && lasthighlight.mouseoverhighlight) if (highlight.priority > lasthighlight.priority) {
...
function addhighlight(idlist, style, force) {
    ...
    for (i = 0; i < markers.length; ++i) if (idlist.includes(markers[i].name)) {
        ...
        if (m.infowindow && m.infowindow.content) m.infowindow.content = m.infowindow.content.replace('value="+"', 'value="*"');
    }
...
function removehighlight(idlist, style) {
    ...
    for (i = 0; i < markers.length; ++i) if (idlist.includes(markers[i].name)) {
        var m = markers[i];
        if (!m.highlight || m.highlight.icon !== style) continue;
        m.highlight.setMap(null);
        m.highlight = null;
    }
}
...
function pdfselect(elem) {
    ...
    opts = "summary=off"; else if (opt == "PM") file = id + "_MAP.pdf", url = base + "filename=" + file + "&pdfx=Map?pagename=" + id + "&ext=.rw", 
    opts = "summary=off"; else if (opt == "KM") file = id + ".kml", url = $("#kmlfilep").html(), 
    count = 1; else if (opt == "GM") file = id + ".gpx", url = base + "gpx=on" + "filename=" + file.split(".")[0] + "&url=" + $("#kmlfilep").html(), 
    count = 1; else if (opt == "ZPM") file = id + ".zip", url = base + "filename=" + file + "&zipx=" + id + "&ext=.rw", 
    opts = "bslinks=off&trlinks=off&summary=off"; else if (opt == "ZALL") file = id + "+.zip", 
    url = base + "filename=" + file + "&zipx=" + id + "&ext=.rw", opts = "bslinks=on&summary=off"; else if (opt == "S") url = SITE_BASE_URL + "/PDFDownload?pagename=" + id + "&ext=.rw";
    url = rwlink(url, opts);
    if (url == "") return;
    elem.value = "";
    elem.blur();
...
function getTableRaps(rapSummary, longestRap) {
    ...
    var longestRapDisplay = '<span class="tablestat rap-longest uft">' + (hasLongest ? "↨" + getTableValueUnit(longestRap) : "&nbsp;") + "</span>";
...
function getUserListTableRow(item) {
    ...
    const EditDelete = '<td class="noprint">' + '<input type="button" value="Edit"   id="[LocationName]-edit"       title="Edit date and comment" onclick="editComment(\'[LocationNameWithoutApostrophe]\')"                class="userlistbutton edit"> ' + '<input type="button" value="⊘" id="[LocationName]-canceledit" title="Cancel the changes"    onclick="cancelEditComment(\'[LocationNameWithoutApostrophe]\')"          class="userlistbutton cancel" style="display:none"> ' + '<input type="button" value="✖" id="[LocationName]-remove"     title="Remove from this list" onclick="removeLocationFromUserList(\'[LocationNameWithoutApostrophe]\')" class="userlistbutton remove"> ' + "</td>";
