...
function waterflow()
{
 ...
 //var boxrect = [ kmlrect[1], kmlrect[0], kmlrect[3], kmlrect[2] ];
 if (!box) return;
 ...
 for (var i=0; i<th.length; ++i)
   if (th[i].id!=null && th[i].id!="")
     ...
     if (th[i].title.substr(0,5)=='http:')
        ...
        th[i].innerHTML = aref(th[i].title, str/*+'<img src="http://ropewiki.com/images/8/80/Camera.png" style="vertical-align:middle"/>'*/, th[i].title, ' target="_blank"');
        }
  ...
  //console.log(url);
   ...
   if (ts[i].values.length>0)
    ...
    for (var i=0; i<sitelist.length; ++i)
      ...
      if (typeof sitelist[i].Gdate != "undefined") 
...
*/
 ...
 function miscgetvalmulti2(val, vallist) 
 {
       ...
       //console.log(val.siteid+"x"+val.date+": "+url);
       $.getJSON(url, function(data) {
 ...
 {  
    ...
    if (msg) 
      ...
      msg.innerHTML = (site.counter>0) ? 
         ...
         (site.error ?  '<img height=12 src="http://ropewiki.com/images/8/87/Vxx.png"/>'+'<span style="color:red">'+site.name+'</span>' : ''); 
         // '<img height=12 src="http://ropewiki.com/images/b/ba/Vxv.png"/>'+site.name);
    ...
    for (c=0; c<sites.length; ++c)
       list.push( {id:sites[c].id, location:sites[c].loc, zindex:sites[c].icon, icon:wficonlist[sites[c].icon], description: sites[c].name.split(",").join(" ").split(";").join(" ")+statuslabel+statusloading } );
    if (map!=null)
       {
       loadlist(list);
       updatemarkers();
...
*/           
      ...
      */
      ...
      //var mulavg = Math.pow(mul, 1/cnt)-1;
      //var sumavg = sum/cnt;
      return sum2/cnt;
      }
                 ...
                 for (var d3=fd; d3<=d; ++d3)
                   if (validrefQG(d3, v3, QG))
                     ...
                     if (newscore<minscore)
                        minscore = newscore, refindex = d, refindex2 = d2, refindex3 = d3, refv3 = v3, refQG = QG;
                     }
                 }
             }
      //refindex = 6; refindex2 = 9; // force refindex
      ...
      for (r=firstcoldate; r<th.length; ++r)
         ...
         if (cell.innerHTML!="" && cell.QG)
             ...
             if (p>=0)
               ...
               if (wflog) str += "<div>Q:"+cell.QG[0].join("|")+"</div>"+"<div>G:"+cell.QG[1].join("|")+"</div>"; // debug
