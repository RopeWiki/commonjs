/* Any JavaScript here will be loaded for all users on every page load. */
  ...
  $j('#highlight-red').hover(
		  function(){
			  $j('#highlight-red').addClass('highlight-red');
		  }, 
		  function(){ 
			  $j('#highlight-red').removeClass('highlight-red');
		  }
  );
  */
  /*
  $('.popup').click(function (event) {
	  ...
	  window.open($(this).attr("href"), "popupWindow", "width=600,height=600,scrollbars=yes");window.reload();
  });
  */
  ...
  function xdeg(slope)
  {
	return Math.round(Math.atan(slope/100.0) * (180/Math.PI),1);
  }
  
  function ftxmi(ftpermile)
  {
	if (metric) 
	  return Math.round(ftpermile/m2ft*km2mi).toLocaleString()+"m/km";
	else
	  return Math.round(ftpermile).toLocaleString()+"ft/mi";
  }
...
"es,Descripción,Aproximación,Descenso,Retorno,Regulaciones,Historia,Créditos,Referencias,Informes del recorrido y fotos,Horario,Rápeles,Combinación,Inicio,Final,Carretera,Parking,(Superior),(Intermedio),(Inferior)",
"pt,Descrição,Aproximação,Descida,Retorno,Regulamentos,História,Créditos,Referências,Relatórios de viagens e fotos,Tempos,Rapeis,Combinação,Entrada,Saída,Estrada,Parking,(Superior),(Intermediário),(Inferior)",
...
"fr,Description,Approche,Descente,Retour,Règlements,Historique,Crédits,Sites internet,Rapports de voyage et photos,Temps,Rappels,Navette,Depart,Arrivee,Route,Parking,(Supérieur),(Intermédiaire),(Inférieur)",
"de,Einleitung,Zustieg,Zejscie,Rückweg,Vorschriften,Hintergrund,Credits,Weblinks,Reiseberichte und fotos,Zeit,Abseilen,Schiffchen,Einleitung,Ausstieg,Strasse,Parking,(Höher),(Intermediär),(Untere)",
"ca,Descripció,Aproximació,Descens,Retorn,Regulacions,Història,Crèdits,Referències,Informes del recorregut i fotos,Horari,Ràpels,Combinació,Inici,Final,Carretera,Parking,(Superior),(Intermedi),(Inferior)"
];
  ...
  */
	 ...
	 // Translated links
	  ...
	  for (var j=0; j<links.length; j++) 
		...
		if (!link.href)
		  continue;
		var url = link.href;
		//if (url.indexOf("ropewiki.com")>=0 || url.indexOf("d5a.net")>=0)
		//   continue;
  ...
  {
	 lastto = loc.join();
	 window.open('http://maps.google.com/?output=classic&saddr='+lastfrom+'&daddr='+lastto,'_blank');
  }
  ...
  if (geoelevationService && LatLng) 
    {
    var latlngs = []; latlngs.push(LatLng);
  ...
  {
	var url ="http://maps.googleapis.com/maps/api/geocode/json?latlng="+lat.toString().trim()+","+lng.toString().trim();
	$.getJSON(geturl(url), function( data ) {
  ...
  if (!div) return;
   ...
   var origurl = geturl("http://ropewiki.com/api.php?format=json&action=query&prop=revisions&rvexcludeuser=BetaRobot&redirects=&continue=");
   switch (mode)
	{
	case 'P':
		  ...
		  function credit(users, size) {
			...
			for (var u = 0; u<users.length; ++u)
				...
				if (users[u].user==addid)
				  ...
				  if (users[u].user.indexOf('http')>0)
				  	{
				  	var userlink = users[u].user.split('http');
				  	credits += userlink[0].link('http'+userlink[1]);
				  	}
				  else
				  	credits += users[u].user.link("http://ropewiki.com/User:"+users[u].user);
  ...
  function loadUserInterface(document){
   ...
   if (tablinks.length>0 && tabmain.length>0) {
	 ...
	 if (tablink1.length>0)
		tabmain[0].innerHTML = '<div class="tabIframeWrapper"><iframe class="tabContent" name="tabIframe2" src="'+tablink1[0].href+'" marginheight="8" marginwidth="8" frameborder="0"></iframe></div>';
   }
  ...
  */
  ...
  /*
   // popupform dialog auto-close
   var interval = setInterval(function(){
   var iframes = document.getElementsByClassName('popupform-innerdocument');
   if (iframes.length>0)
	 {
	   //console.log("popupform-innerdocument");
	   var iframe = iframes[0];
	   var subdoc = iframe.contentDocument || iframe.contentWindow.document;
	   var uploadfiles = subdoc ? $(subdoc).find('#wpUploadFile') : null;
	   var uploadfile = (uploadfiles && uploadfiles.length>0) ? uploadfiles[0] : null;
  
	   if (uploadfile && uploadfile.value!=null && uploadfile.value!="" && !iframe.getAttribute("uploadset"))
		  {
		  iframe.setAttribute("uploadset", "on");
		  console.log("auto-upload");
		  $('.popupform-loadingfg').show();
		  $('.popupform-loadingbg').show();
		  $('.popupform-loading').show();
		  $('.popupform-loading').css({zIndex: 1500});
		  $('.popupform-container').css({opacity: 0.5});
		  $(subdoc).find('.mw-htmlform-submit').show().focus().click();
		  }
  
		 if (uploadfile && (uploadfile.value==null || uploadfile.value=="") && !iframe.getAttribute("browseset"))
		  {
		  iframe.setAttribute("browseset", "on");
		  //var interval = setTimeout(function(){
		  console.log("auto-browse");
		  $(subdoc).find('#wpUploadFile').show().focus().click();
		  //uploadfile.click();
		  //}, 1000);
		  }
  
		var src = iframe.contentWindow.location.href;
		if (src.search('/File:')>=0 && !iframe.getAttribute("urlset"))
		  {
		  iframe.setAttribute("urlset", "on");
		  console.log("auto-close");
		  $('.popupform-close').click();
		  window.location.reload(true);
		  }
  
		if (!iframe.getAttribute("loadset"))
		{
		iframe.setAttribute("loadset", "on");
		var id = '#'+iframe.id;
		//console.log("load-iframeName "+id);      
		$(id).load(function() {
		 console.log("src:"+iframe.contentWindow.location.href);
		  $('.popupform-loading').hide();
		  $('.popupform-container').css({opacity: 1});
		 //if (subdoc.getElementsByClassName('mw-htmlform-field-UploadSourceField').length>0)
		});
		}
	 }
   }, 1000);
  */
   ...
   for (var i = 0; i < months.length; i++) 
	  ...
	  for (var s=0; s<str.length; ++s)
		 switch (str.charAt(s)) 
		 {
		   case ',': out += '<td class="bss"></td>'; break;
		   case 'X': out += '<td class="bsg"></td>'; break;
		   case 'x': out += '<td class="bsy"></td>'; break;
		   case '.': out += '<td></td>'; break;
		 }
	  out += '</tr></table>';
  ...
  for (var i = 0; i < elem.length; i++)  {
	  ...
	  elem[i].innerHTML = '<input id="'+elem[i].id+'" class="gmnoprint" type="checkbox" onclick="toggleUrlcheckbox(this)" '+(on=='on' ? 'checked' : '')+ '>';
	}
  ...
  if (hdr && pdf && kml&& !edit){
   ...
   for (var i = 0; i < lines.length; i++) 
	 ...
	 if (lines[i].id) {
	   ...
	   if (idlist.length>1)
		 icon = kmliconlist[ Number(idlist[1])*6 + Number(idlist[0]) ];
	 }
   ...
   if (coords.length>0 && weatherdiv && weather)
   {
		...
		if (staticdebug) // debug
			url = "http://localhost/rw/umpqua.json";
 		$.getJSON(geturl(url), function( data ) {
			// forecast.txt_forecast.forecastday.   period, icon_url, title, metric ? fcttext_metric : fcttext  Night
			// simpleforecast 

		  if (data && data.forecast && data.forecast.simpleforecast && data.forecast.simpleforecast.forecastday && data.forecast.simpleforecast.forecastday.length>0)
			{
			var link;
...
//weatherchk.innerHTML += '<div class="wstlogo noprint">Powered by<br><img src="https://icons.wxug.com/logos/PNG/wundergroundLogo_4c_horz.png"/></span></div>';
      ...
      var station, station2;
			if (data.location && data.location.nearby_weather_stations && data.location.nearby_weather_stations.pws && data.location.nearby_weather_stations.pws.station && data.location.nearby_weather_stations.pws.station.length>0)
				{
			  station = data.location.nearby_weather_stations.pws.station[0];
			  station.name = "((&#9899;)) "+station.id;
				}
			if (data.location && data.location.nearby_weather_stations && data.location.nearby_weather_stations.airport && data.location.nearby_weather_stations.airport.station && data.location.nearby_weather_stations.airport.station.length>0)
				{
			...
			if (ll.length>=2)
			   ...
			   if (station2)
			     if ((distance2 = Distance({lat:ll[0], lng:ll[1]}, {lat:station2.lat, lng:station2.lon}))<distance)
			   ...
			   w += "~"+mi(distance)+" ";
 			    ...
 			    var infowindow = new google.maps.InfoWindow({ content: '<b>'+station.name+'</b>'+'<div class="textselect">'+stationinfo.join(', ')+'</div>'+'<div class="textselect">'+displaylocation(station.lat, station.lon)+'<div id="elevation"></div>'+'</div>'});
	  			var marker = new google.maps.Marker({ position: myLatlng,  map: map, infowindow:infowindow, optimized: false, zIndex:10, icon:{url:"https://icons.wxug.com/logos/PNG/wundergroundLogo_4c.png", scaledSize: new google.maps.Size(45, 32)} });  //"http://ropewiki.com/images/f/f2/Wunderground.png"
	  			google.maps.event.addListener(marker, 'click', function() { this.infowindow.open(map, this); getgeoelevation(this.getPosition(), "elevation", "Elevation: "); });
	  			}
			   }
			if (station)
				w += ' '+station.name;
   ...
   if (maptype && (maptype.innerHTML=='topo' || staticdebug))  // USA & Canada
   {
   	function extractVal(str, label)
   	{
		...
		if (staticdebug)
			url ="http://d5a.net/rwr?waterflow=winfo="+"33.7717,-111.1197";
 		$.getJSON(geturl(url), function( data ) {
			// forecast.txt_forecast.forecastday.   period, icon_url, title, metric ? fcttext_metric : fcttext  Night
			// simpleforecast 
		  ...
		  if (data && data.list && data.list.length>0)
			{
				...
				if (!isNaN(num))
					 ...
					 if (num>100)
				   		w += '<hr id="cd5" title="May massively flood with heavy rains and keep flooded for many days afterwards">';
					 else if (num>50)
   ...
   for (var i = 0; i < boxes.length; i++) 
	 {
	  ...
	  boxes[i].onmouseover= function() {
		...
		isrc = 'http://ropewiki.com/'+imgsrc[0];
		*/
		// use a highres div      
