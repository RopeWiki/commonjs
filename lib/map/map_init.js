...
// cover
    ...
    var relief = new google.maps.ImageMapType({
        getTileUrl: function (p, z) {
            return "http://s3-us-west-1.amazonaws.com/ctrelief/relief/" + z + "/" + slippyClip(p.x, z) + "/" + slippyClip(p.y, z) + ".png"
    ...
    map.mapTypes.set("topo", new google.maps.ImageMapType({
        getTileUrl: function (p, z) {
            return "http://s3-us-west-1.amazonaws.com/caltopo/topo/" + z + "/" + slippyClip(p.x, z) + "/" + slippyClip(p.y, z) + ".png"
    ...
    map.mapTypes.set("ocm", new google.maps.ImageMapType({
        getTileUrl: function (p, z) {
            return "http://tile.opencyclemap.org/cycle/" + z + "/" + slippyClip(p.x, z) + "/" + slippyClip(p.y, z) + ".png"
...
//  credits["wtopo"] = "<a href='http://oprntopomap.org' target='_blank'>Topo map by OpenTopoMap</a>";
    map.mapTypes.set("wtopo", new google.maps.ImageMapType({
        getTileUrl: function (p, z) {
            return "http://tile.thunderforest.com/outdoors/" + z + "/" + slippyClip(p.x, z) + "/" + slippyClip(p.y, z) + ".png?apikey=bdbb04f2d5df40cbb86e9e6e1acff6f7"
    ...
    map.mapTypes.set("wtopoesri", new google.maps.ImageMapType({
        getTileUrl: function (p, z) {
            return "http://services.arcgisonline.com/ArcGIS/rest/services/World_Topo_Map/MapServer/tile/" + z + "/" + slippyClip(p.y, z) + "/" + slippyClip(p.x, z) + ".png"
    ...
    map.mapTypes.set("wsatesri", new google.maps.ImageMapType({
        getTileUrl: function (p, z) {
            return "http://services.arcgisonline.com/ArcGIS/rest/services/World_Imagery/MapServer/tile/" + z + "/" + slippyClip(p.y, z) + "/" + slippyClip(p.x, z) + ".png"
    ...
    map.mapTypes.set("estopo", new google.maps.ImageMapType({
        getTileUrl: function (p, z) {
            return "http://www.ign.es/wmts/mapa-raster?SERVICE=WMTS&REQUEST=GetTile&VERSION=1.0.0&LAYER=MTN&STYLE=default&TILEMATRIXSET=GoogleMapsCompatible&TILEMATRIX=" + z + "&TILEROW=" + slippyClip(p.y, z) + "&TILECOL=" + slippyClip(p.x, z) + "&FORMAT=image%2Fjpeg"
    ...
    map.mapTypes.set("frtopo", new google.maps.ImageMapType({
        getTileUrl: function (p, z) {
            return "http://RopeWiki:b%40VM709c6Jix@wxs.ign.fr/8m34uuspdybhi9t89lpoeyfl/geoportail/wmts?SERVICE=WMTS&VERSION=1.0.0&REQUEST=GetTile&LAYER=GEOGRAPHICALGRIDSYSTEMS.MAPS&STYLE=normal&FORMAT=image/jpeg&TILEMATRIXSET=PM&TILEMATRIX=" + z + "&TILEROW=" + slippyClip(p.y, z) + "&TILECOL=" + slippyClip(p.x, z)
    ...
    map.mapTypes.set("mxtopo", new google.maps.ImageMapType({
        getTileUrl: function (p, z) {
            return "http://gaiamapas1.inegi.org.mx/mdmCache/service/wms?LAYERS=MapaBaseTopograficov61_consombreado&FORMAT=image/jpeg&MINZOOMLEVEL=5&ZOOMOFFSET=5&SERVICE=WMS&VERSION=1.1.1&REQUEST=GetMap&SRS=EPSG:900913&BBOX=" + WmsBox(latLngBox(slippyClip(p.x, z), slippyClip(p.y, z), z, 256), true, false) + "&WIDTH=256&HEIGHT=256"
    ...
    map.mapTypes.set("nztopo", new google.maps.ImageMapType({
        getTileUrl: function (p, z) {
            return z < 13 ? "http://tiles-2.topomap.co.nz/tiles-topo250/" + z + "-" + slippyClip(p.x, z) + "-" + TYZ(slippyClip(p.y, z), z) + ".png" : "http://tiles-2.topomap.co.nz/tiles-topo50/" + z + "-" + slippyClip(p.x, z) + "-" + TYZ(slippyClip(p.y, z), z) + ".png"
