...
function loadInteractiveMap() {
    ...
    var mapOptions = {
        ...
        */
        ...
        mapTypeControlOptions: {
            mapTypeIds: [
                google.maps.MapTypeId.TERRAIN, 'roads', 'topo', 'wtopo', 'wtopoesri', 'ocm', google.maps.MapTypeId.HYBRID, 'wsatesri'
                //, 'mxtopo', 'estopo', 'frtopo', 'chtopo', 'nztopo'
            ],
    ...
    // Open Street Map version of roads
    //map.mapTypes.set("roads",
    //    new google.maps.ImageMapType({
    //        getTileUrl: function(p, z) {
    //            return "http://tile.openstreetmap.org/" + z + "/" + CLIP(p.x, z) + "/" + CLIP(p.y, z) + ".png"
    //        },
    //        maxZoom: 16,
    //        name: "Roads",
    //        opacity: 1,
    ...
    map.mapTypes.set("roads",
        new google.maps.ImageMapType({
            getTileUrl: function(p, z) {
                return "https://mt0.google.com/vt/lyrs=m@134&hl=en&src=api&x=" +
                    slippyClip(p.x, z) +
                    "&y=" +
                    slippyClip(p.y, z) +
                    "&z=" +
    ...
    map.mapTypes.set("frtopo",
        new google.maps.ImageMapType({
            getTileUrl: function(p, z) {
                return "http://RopeWiki:b%40VM709c6Jix@wxs.ign.fr/8m34uuspdybhi9t89lpoeyfl/geoportail/wmts?SERVICE=WMTS&VERSION=1.0.0&REQUEST=GetTile&LAYER=GEOGRAPHICALGRIDSYSTEMS.MAPS&STYLE=normal&FORMAT=image/jpeg&TILEMATRIXSET=PM&TILEMATRIX=" +
                    z +
                    "&TILEROW=" +
                    slippyClip(p.y, z) +
                    "&TILECOL=" +
    ...
    map.mapTypes.set("mxtopo",
        new google.maps.ImageMapType({
            getTileUrl: function(p, z) {
                return "http://gaiamapas1.inegi.org.mx/mdmCache/service/wms?LAYERS=MapaBaseTopograficov61_consombreado&FORMAT=image/jpeg&MINZOOMLEVEL=5&ZOOMOFFSET=5&SERVICE=WMS&VERSION=1.1.1&REQUEST=GetMap&SRS=EPSG:900913&BBOX=" +
                    WmsBox(latLngBox(slippyClip(p.x, z), slippyClip(p.y, z), z, 256), true, false) +
    ...
    map.mapTypes.set("nztopo",
        new google.maps.ImageMapType({
            getTileUrl: function(p, z) {
                return z < 13
                    ? "http://tiles-2.topomap.co.nz/tiles-topo250/" +
                    z +
                    "-" +
                    slippyClip(p.x, z) +
                    "-" +
                    TYZ(slippyClip(p.y, z), z) +
                    ".png"
                    : "http://tiles-2.topomap.co.nz/tiles-topo50/" +
                    z +
                    "-" +
                    slippyClip(p.x, z) +
                    "-" +
    ...
    map.mapTypes.set("chtopo",
        new google.maps.ImageMapType({
            getTileUrl: function(p, z) {
                return "http://wmts20.geo.admin.ch/1.0.0/ch.swisstopo.pixelkarte-farbe/default/current/3857/" +
                    z +
                    "/" +
                    p.x +
                    "/" +
    ...
    // set map type from "kmltype" <--commented out by Michelle -- no longer doing custom topo maps worldwide
    ...
    // add special controls
    {
        ...
        var controls = spstart +
            '<img class="gmnoprint" id="fullscreenchk" onclick="toggleFullScreen()" title="Full Screen" src="' +
            SITE_BASE_URL +
            '/images/b/b9/FullscreenIcon.png">' +
            spend;
        if (kmllist) {
            /*
            // Cluster Engine
            ...
            */
            ...
            controls += spstart +
                '<label><input class="gmnoprint" id="routeschk" type="checkbox" onclick="toggleRoutes()">Show&nbsp;KML&nbsp;</label>' +
                spend;
